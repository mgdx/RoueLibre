...
- The tile file is **not in the APK**: it is downloaded on first launch (see §4.5). Expected order of magnitude for a medium-sized conurbation: **30 to 60 MB**. A dense metropolis legitimately produces more — Paris, with 1.24 million building footprints inside its box against 78,000 for Lille, produces 115 MB. The ceiling is **200 MB**, and it applies to the base map alone, the heaviest of the three sets. Under it, no city is refused and no per-city exception is carved out: the rendering rules stay the same everywhere. Over it, the layer to pull is stated below — building footprints — and pulling it changes the map of every city, which is the point: a rule that holds for one conurbation only is not a rule.

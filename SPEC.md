...
**The availability figure can be asked for larger, and that is a setting of its own** (§7.6). It does not stand in for the system text size, which the whole interface already follows: it does what that setting cannot, which is to enlarge the one figure looked at a hundred times a week without enlarging everything around it. It reaches the figure where it is read at leisure — the rows of the station list, the favourites, a station's sheet — and **not the map's markers**: a marker's size decides how many stations stay legible side by side at a given zoom, which is a question of map drawing rather than of accessibility, and enlarged the discs would overlap and the map would say less. The disc grows by the same fraction as the figure inside it, at either size, since it is that ratio and not the absolute size that keeps three digits inside the ring.
...
- **Two filters on what is drawn at all**, the sequel to the toggle above: it says what a marker counts, they say which markers there are. **Hide the stations out of service** and **hide the empty ones**, "empty" reading with the toggle — no bike while bikes are counted, no free dock while docks are — so the same station is dropped by one mode and kept by the other. **Both are off by default**: a map nobody has asked anything of shows the network as it stands. There is **no adjustable threshold** and no "at least three bikes": one more figure to justify, one more control on a screen §7 asks to keep calm, and the marker already says how many there are.
  **What is unknown is not what is known to be absent.** "Out of service" hides the stations the feed says are out of service and **never** the ones it says nothing about; "empty" hides a count read as zero and **never** a count nobody could read — including a breakdown by kind that could not be trusted (§7.2). Hiding either would assert, on the strength of a silence, something the application has not read, and the state of the Lille feed — 268 stations on one side, 267 on the other — is exactly the silence in question. The rule is stated in the sheet where the switches are set.
  **A map that hides stations says so, without anything being opened.** While a filter is on, the control naming it says how many stations are missing right now, nought included: somebody finding a quarter empty must understand why in a second. The filters are **kept for as long as the screen and written nowhere**, like the toggle and the kind filter beside them — a way of looking at the map for a minute, not a fact about the rider — which is also what keeps a filter from becoming a trap laid for the next launch.
  **The stations of a journey drawn on the map are never hidden**: the application has just proposed them, and making them vanish would be the screen contradicting itself. Stations beyond the reference box (§4) are not concerned either — these filters speak of state and of counts, and being outside the data is neither.
...
**A section holding nothing is not shown, and is not written into the layout either.** A title followed by nothing promises a setting the screen does not have, and §14 wants no view built against a feature that has not arrived. "Journey" was in that case when this was written, and it stopped being so on 16 August 2026: the walking pace, the first setting placed there, brought its title with it, between the city and the display. What a setting costs is therefore its own views under the right title, and nothing above or below them.
...
- **What the rider's own bike is** comes next in the journey section: **not specified, mechanical, electric**, and **not specified is the default**, at installation, after any reset and for a word that cannot be read. Not specified reproduces exactly the drawings and the sentences of the version before this choice existed, so nothing changes for somebody who never opens this screen.
  ...
  - **It changes only what is drawn and what is said**: the two ends of a journey on one's own bike take the bolt (§7.3), and the summary says "on your own electric bike" (§7.4). **No speed, no coefficient, no dedicated profile**, and this is a decision rather than an omission: a pedal-assist bike is quicker in the real world, but §6 announces only what the routing engine traced, and the ride runs over the same graph with the same profile whatever was declared. The same pair of points therefore comes back with the same track and the same minutes under all three states. What holds that is structural — the type lives in the application's settings, and the module the algorithm lives in cannot see it.
...
- **The screen the application opens on**, beside them: the map or the station list. **The map is the default**, at installation and after any reset, and a stored value that cannot be read means the map too — it is what §7.0 opens on and what somebody who never comes here goes on seeing. The list earns its place because the map is not what everybody opens the application for: somebody who always sets off from the same station reads one line of a list, not a plan. It is read once, by the activity, at the launch after the choice; nothing on screen moves when it is pressed, unlike the theme and the units.
  ...
  - **The list opened on carries the way to the map**, which it does not carry when it was reached from one: there it has a map behind it and the back gesture is the way to it, whereas opened on it has nothing behind it, and the map is where the journey search and the settings are reached from — without that way out, the setting that put the user there could not be undone.
- **Larger availability figures**, last of the display section: a switch, **off by default** and off again for a preference that was never written. What it does and where it stops are described in §7 — the list, the favourites and a station's sheet, never the map's markers. It is written the moment it is pressed, and the screens showing an indicator follow the stored value themselves, so a figure changes size on the list one goes back to without the application being restarted. The line under the switch says what the map keeps, so that nobody looks for a change that was decided against.

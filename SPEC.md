...
- The **fleet** is counted from the network's own feeds by `tools/read_fleet.py`, never typed in and never taken from the `vehicle_types` declaration alone — a third of the networks declaring a mixed fleet have not one bike of one of the two kinds out (§7). The script reads the declaration to know what each vehicle type identifier is, then counts the bikes standing at the stations in `station_status`, through `vehicle_types_available` or through Vélib's `num_bikes_available_types`. It writes three things: whether pedal-assist bikes are in circulation, which decides the bolt (§7); whether both kinds are, which decides the split on a station's sheet (§7.2); and the table saying which identifier is mechanical, which is electric and which is neither a bike at all, without which that split cannot be read.
...
- The application itself never calls the `vehicle_types` feed: what a network lends is a fact settled when the city is added, not real-time state. Only the numbers are real-time, and they travel in the `station_status` feed already fetched.

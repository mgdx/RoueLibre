...
- a bike bearing a **bolt and a cog** where it lends both side by side — the cog set under the bolt and a little towards the edge, so the two read as one pair rather than as two marks: what a motor helps to pedal, and what one pedals alone.

Not a brand mark and not a decoration: it is the difference between three offers, and it is the first thing one wants to know before walking to a station. Which of the three applies is read from the city configuration (§15) and from nowhere else, so a mechanical fleet in one conurbation and a mixed one in the next need no code change. The badges are drawn inside the glyph, at the glyph's own scale, so they ride down to marker size with it rather than being pinned to a corner of the disc. The cog's size is settled by the drawing and not by taste: the largest disc that fits under the bolt without touching the front wheel has a radius of 2.35 in a 24-unit glyph, and the cog is drawn at 2.0 to keep a gap on every side. The application's own identity is untouched — the launcher icon and the welcome screens are the same whichever city is served, and one of them is shown before any city has been chosen at all.

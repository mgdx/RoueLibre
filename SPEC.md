...
- **The same total and the same summary as the screen it comes from**, word for word and from the same resources: a total worded differently would read as a second journey. The drawing follows, since it is what the press was made on.
- Then the journey **place by place and leg by leg, in the order it is lived**: the start, each walk and the ride with its distance, its minutes and its climb, and between them the two stations. It is one reading, from where one stands to where one is going, not a table of legs on one side and stations on the other.

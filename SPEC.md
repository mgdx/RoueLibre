...
- **The language** the interface speaks, under the units and settled by §9: following the system by default, and applied the moment it is chosen, with no "apply" button — the screens are rebuilt on it, as they are on a new theme and by the same mechanism. It is **a row that opens a list**, and not a row of buttons like the two settings above it: the choices are the translations that exist, they grow every time somebody finishes one, and each is written in its own language — "Français", "العربية" — which no three-across row of buttons would hold. The row carries the language in service, so the screen answers the question without being pressed, and its spoken label names the setting as well as the value, "Français" alone saying nothing about what it settles. **The list is derived from the translations that exist** and never written out beside them (§9), and the same choice is offered by Android's own per-application language settings, writing the same stored value.

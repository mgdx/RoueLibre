...
leses fra nettverkets eget feed"**. Norwegian's *det*-passive and s-passive do
...
| network (bike-share) | nettverk | The operator whose bikes these are. It is never used of a data connection — see the next row — so `error_malformed` can say "feeden til nettverket" without a reader wondering which network is meant. |

...
| an address, as written | Straße, Hausnummer, Ort | German writes the street before the number, and that order now holds on both sides of the screen: `address_search_hint` asks for it that way, and `address_with_number` prints it that way (`%2$s %1$s`, the placeholders swapped — that is what they are positional for). `AddressQuery.parseQuery` reads a number standing between street and town since 9510db7c, so the German order is understood as typed. The **postcode is deliberately left out of the hint**: the parser only strips one written as a single group of five digits, which is Germany but neither Austria nor Switzerland, and a second number in the query makes the house number be given up. Asking three German-speaking countries for a postcode would break the reading for two of them. |

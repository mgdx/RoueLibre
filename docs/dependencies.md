...
| **MapLibre Native** | offline vector map | The project's only native dependency, and its only accepted departure from the size constraint: it is the price of offline operation. Reads the MBTiles straight from disk, without a tile server. BSD-2-Clause, minSdk 23. Shipped as the **`android-sdk-opengl` artefact**: since MapLibre 13 the default `android-sdk` renders through Vulkan only and marks Vulkan 1.0 as a required manifest feature, which would refuse installation on API 26+ devices without it; the OpenGL ES artefact is the same engine, same version, without that requirement — and its native library is about two megabytes lighter per ABI. **When raising its version, check that `maplibre_logo_icon` and `maplibre_compass_icon` still carry those names**: both maps turn the library's logo and compass off, but `MapView` loads the pictures before reading the flags, so the application ships an empty file under each of those names at each density. A rename in the library leaves the stubs covering nothing and puts sixty-eight kilobytes of invisible pictures back into the APK, without an error to show for it. |
...
| **Bricolage Grotesque** | figures typeface | Bike counts are the central information; they deserve letters recognisable from a distance. Frozen into two static instances. SIL OFL. The bold one is **cut down to the nineteen characters it draws** — the ten figures, a space, `. , + -`, `k K M` for the abbreviated cluster counts and `?` for a station whose count is unknown — which takes it from 91 kB to 14 and the map glyph range built from it from 77 kB to 7. It writes the availability indicator and the markers' counts and nothing else; using it for a word would show that word blank, so a new use of it means widening the subset and rebuilding the glyphs with `npm run glyphs`. |

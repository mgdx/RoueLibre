...
| My places | Os meus locais | The settings section that holds the two (`settings_section_places`). The article before the possessive, as European Portuguese writes it and as the rest of the file does. *Local* is the noun `place_sheet_title` uses as well. |
| outside the city served | Fora da cidade coberta | Under a place the conurbation in use does not cover (`settings_place_outside_city`). *Fora de* and *cobrir* are what `station_beyond_area` already says. The application serves several conurbations side by side: a *Casa* named in Lille is out of reach while Lisboa is in use. The place is kept and stays erasable — out of reach, not wrong. |
| Face north and lay the map flat | Orientar para norte e aplanar o mapa | `map_face_north`, on the compass, which shows while the map is turned or tilted, and one press gives back both at once — the north and the flat; `map_bearing_description` counts the turn in *graus*. |

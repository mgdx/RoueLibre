...
| Home (the place) | Hjem | Where the reader lives, named by them (`settings_place_home`, `journey_source_home`, the same word in both). Not an application's first screen, which is *startskjermen* — a different word for a different thing. |
| Work (the place) | Jobb | Where they work (`settings_place_work`, `journey_source_work`). *Jobb* is the place one goes to; *Arbeid* would be the activity, and it is not a point on a map. It stands beside *Hjem* under *Stedene mine*, which settles the reading. |
| My places | Stedene mine | The settings section that holds the two (`settings_section_places`). The possessive after the noun, as Norwegian writes it; *sted* is the noun `place_sheet_title` uses as well. |
| outside the city served | Utenfor byen som dekkes | Under a place the conurbation in use does not cover (`settings_place_outside_city`). A relative clause rather than the participle *den dekkede byen*, which is bookish, and *utenfor … dekker* is the frame `station_beyond_area` already uses. The application serves several conurbations side by side: a *Hjem* named in Lille is out of reach while Oslo is in use. The place is kept and stays erasable — out of reach, not wrong. |
| Face north and lay the map flat | Vend mot nord og legg kartet flatt | `map_face_north`, on the compass, which shows while the map is turned or tilted, and one press gives back both at once — the north and the flat; `map_bearing_description` counts the turn in *grader*. |

...
| ride | cykeltur | The bike leg alone, inside a journey: `journey_computing_own_bike`, `journey_detail_profile`, `journey_hint_own_bike`. Plain *tur* was not used, because Swedish gives *tur* a whole outing as readily as one leg — and a departure on a timetable besides. Where the ride is a manner rather than an object, the file says **på cykel** ("varav 12 min till fots och 9 min på cykel") — including in `journey_walk_only` and its `_climb` twin, "det går inte att göra på cykel här", which go round the noun rather than coin a third one. **Do not write *cykelresa* there**: it stood in the file once, and it put a third object between *resa* and *cykeltur* on the very screen where both are read. |
...
| electric bike, as an object | elcykel | **A deliberate departure from the line above, in five strings.** *Elcykel* is the noun Swedish has for the thing — the word the law, the shops and the riders use — and "på din egen elektriska cykel" reads as a translation where "på din egen elcykel" reads as Swedish. So the adjective labels the *choice* (`settings_own_bike_kind_electric` = "Elektrisk", `map_bikes_electric` = "Elektriska") and the noun names the *object*: `journey_own_bike_electric_only`, its `_climb` twin, `journey_bike_kind_electric_description` = "En elcykel med trampassistans", `welcome_fleet_electric_only`, and `journey_no_electric_nearby` — "Ingen station i närheten har en **elcykel** just nu". **That last one breaks a parallel on purpose.** Its twin `journey_no_mechanical_nearby` says "en **mekanisk cykel**", adjective plus noun, where the English pair is symmetrical; the two sit on one screen and answer the same failure. Swedish has no noun for a mechanical bike the way it has *elcykel* for the other, and writing "en elektrisk cykel" there to keep the shapes matching would be the one place in the file where the object is named by the adjective. Keep the pair as it stands. That description also settles what "electric" means here: *trampassistans* is Transportstyrelsen's own term for a pedelec, and rules out a moped. Do not "correct" the four back to the adjective. |
...
| hour (in a duration) | tim | `duration_hours_minutes` is `%1$d tim %2$02d`. **This is this file's own choice and no key attests it**: the extract holds *timme* and *timmar* and never the abbreviation, and the preface's rule is to say so. *tim* is the abbreviation Swedish dictionaries give, and `h` reads as foreign outside *km/h*. It is the one unit symbol this file changes. The padded minute after it — "1 tim 05" — is a clock idiom rather than a Swedish habit, which would write "1 tim 5 min"; it is kept because Danish and Norwegian carry the same shape with *t*, and one language breaking the Nordic row alone would buy less than it costs. |
| Home (the place) | Hem | Where the reader lives, named by them (`settings_place_home`, `journey_source_home`, the same word in both). Not an application's first screen, which is *startsidan* — a different word for a different thing. |
| Work (the place) | Jobb | Where they work (`settings_place_work`, `journey_source_work`). *Jobb* is the place one goes to; *Arbete* would be the activity, and it is not a point on a map. It stands beside *Hem* under *Mina platser*, which settles the reading. |
| My places | Mina platser | The settings section that holds the two (`settings_section_places`). *Plats* is the noun `place_sheet_title` uses as well. |
| outside the city served | Utanför den täckta staden | Under a place the conurbation in use does not cover (`settings_place_outside_city`). *Utanför* and *täcka* are what `station_beyond_area` already says. The application serves several conurbations side by side: a *Hem* named in Lille is out of reach while Stockholm is in use. The place is kept and stays erasable — out of reach, not wrong. |
| Face north and lay the map flat | Rikta mot norr och lägg kartan platt | `map_face_north`, on the compass, which shows while the map is turned or tilted, and one press gives back both at once — the north and the flat; `map_bearing_description` counts the turn in *grader*. |
...
- `dataset_absent` = **"Saknas"**, which is invariable, is what a Swedish row
  ...
  **This departs from Android, and the departure is the point.** The phone does
  have the participle — `settings:installed` = "Installerad",
  `settings:not_installed` = "Ej installerad för användaren" — but it is the
  common-gender form, right for the *app* Android is describing there and wrong
  after *Adressindex*. Taking Android's word would have meant choosing a gender
  for the three names, which is exactly what this row exists to avoid.

Android writes the same *s*-passive elsewhere, which is worth knowing before
anyone "corrects" it back to a participle: `settings:credential_for_wifi` =

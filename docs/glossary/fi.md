...
| Face north | **Suuntaa pohjoiseen** | `map_face_north`, on the compass, which shows only while the map is turned; `map_bearing_description` counts the turn in degrees, and its two forms carry the two shapes a Finnish numeral governs, *asteen* and *astetta*. |

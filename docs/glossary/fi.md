...
| unmetered / metered | **maksuton / maksullinen** | Android labels a connection *Maksuton* / *Maksullinen* (`settings:wifi_unmetered_label`, `settings:wifi_metered_label`). **The switch itself no longer uses the pair**: it reads *Lataa vain Wi-Fi-verkossa*, as the English now does, Wi-Fi being the word every reader has. The pair stays here as the name of what the code reads, and the sentences under the switch keep speaking of billing — *voidaan laskuttaa megatavuittain* — since that is the point the English makes. |
| may be billed | **voidaan laskuttaa** | Never the flat *laskutetaan*. The phone reads only that the connection declares itself metered; what the subscription behind it charges, no device can know, and the passive with *voidaan* says exactly that much. `download_held_back_title` names the situation for the same reason — *Lataus mobiiliverkossa*, not a verdict on the connection — and `download_can_resume` drops the modal because there the device does state what it knows: *Tämä yhteys ei ole enää mobiiliverkko*. |
...
| Home (the place) | **Koti** | Where the reader lives, named by them (`settings_place_home`, `journey_source_home`, the same word in both). Not an application's opening screen, which Finnish calls *aloitusnäyttö*. |
| Work (the place) | **Työ** | Where they work (`settings_place_work`, `journey_source_work`). The place, not the occupation: it stands beside *Koti* under *Omat paikat*, which settles the reading; *Työpaikka* names the workplace outright and is twice the width on a row that already carries an address. |
| My places | **Omat paikat** | The settings section that holds the two (`settings_section_places`). *Omat* rather than *Minun*, which is what Finnish writes for a section of one's own things; *paikka* is the noun `place_sheet_title` uses as well. |
| outside the city served | **Katetun kaupungin ulkopuolella** | Under a place the conurbation in use does not cover (`settings_place_outside_city`). The same frame as `station_beyond_area`, *kattaman alueen ulkopuolella*: the verb *kattaa* and the postposition *ulkopuolella*. The application serves several conurbations side by side: a *Koti* named in Lille is out of reach while Helsinki is in use. The place is kept and stays erasable — out of reach, not wrong. |
| Face north and lay the map flat | **Suuntaa pohjoiseen ja tasaa kartta** | `map_face_north`, on the compass, which shows while the map is turned or tilted, and one press gives back both at once — the north and the flat; `map_bearing_description` counts the turn in degrees, and its two forms carry the two shapes a Finnish numeral governs, *asteen* and *astetta*. |

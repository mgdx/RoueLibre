...
| Reserved (a bike) | محجوزة | `station_bike_reserved`, the one word a line of the station sheet's bike list adds where the feed says the bike is booked. The passive participle of حجز, Android's own verb for a booking, in the feminine because it qualifies دراجة: the word stands on a line that names one bike. |
...
| Face north and lay the map flat | التوجيه نحو الشمال وتسوية الخريطة | `map_face_north`, on the compass, which shows while the map is turned or tilted, and one press gives back both at once — the north and the flat; the masdar, as the file's controls are. `map_bearing_description` counts the turn in درجة and carries all six categories, `two` and `few` included. |
| tilt / lay flat | إمالة / مائلة, تسوية | `map_description` says واسحب إصبعيك لأعلى أو لأسفل لإمالتها for the gesture, `map_tilted_description` الخريطة مائلة for the state, and `map_face_north` تسوية for putting the map back flat. **مائلة now names the tilt and nothing else**: the bearing strings said الخريطة مائلة عن الشمال for a *turn*, which would have made one word carry two of the four gestures, and they were reworded to الخريطة مُدارة عن الشمال. |

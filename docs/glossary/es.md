...
| metered connection | conexión de uso medido | Android's own wording for what the code actually reads. **The switch no longer says it**: it reads *Descargar solo con Wi-Fi*, as the English now does, because Wi-Fi is the word every reader already has. The pair stays here as the name of the concept, and the line under the switch and the two dialogs explain it in ordinary words — *facturarse por megabyte*, *cuenta como una red móvil*. |
| may be billed | puede facturarse | Never *se factura*. The phone reads only that the connection declares itself metered; what the plan behind it charges, no device can know. So `download_unmetered_only_description`, `download_held_back_body` and `download_stopped_body` all carry *puede*, and `download_held_back_title` names the situation — *Descarga en una red móvil* — rather than passing a verdict on the connection. `download_can_resume` drops the modal because there the device does state what it knows: *ya no es una red móvil*. |
...
| Face north and lay the map flat | Orientar al norte y aplanar el mapa | `map_face_north`, on the compass, which shows while the map is turned or tilted, and one press gives back both at once — the north and the flat; `map_bearing_description` counts the turn in *grados*. |
| tilt / lay flat | inclinar / inclinado, aplanar | *Inclinar* is the tilt and *girar* the turn, kept apart so that the four gestures of `map_description` stay four. `map_tilted_description` is «El mapa está inclinado.», and `map_face_north` says *aplanar* for laying it flat again. In `map_bearing_tilted_description` the conjunction is **e**, not *y*, before *inclinado*. |

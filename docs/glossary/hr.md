...
| Wi-Fi | **Wi-Fi** | Untranslated, and declined as an ordinary masculine: *na Wi-Fiju* on the switch (`download_unmetered_only`), *s Wi-Fijem* in the two dialogs. The switch names Wi-Fi rather than the billing, as the English does, because it is the word every reader already has; the billing is what the line under it explains. |
| may be billed | **može se naplaćivati** | Never the flat *naplaćuje se*. The phone reads only that the connection declares itself metered; what the plan behind it charges, no device can know. `download_held_back_title` names the situation for the same reason — *Preuzimanje na mobilnoj mreži*, not a verdict on the connection — and `download_can_resume` drops the modal because there the device does state what it knows: *Ova veza više nije mobilna mreža*. |
...
| Home (the place) | **Dom** | Where the reader lives, named by them (`settings_place_home`, `journey_source_home`, the same word in both). The dwelling, never an application's opening screen. |
| Work (the place) | **Posao** | Where they work (`settings_place_work`, `journey_source_work`). *Posao* is the place one goes to; *Rad* would be the work itself, and it is not a point on a map. |
| My places | **Moja mjesta** | The settings section that holds the two (`settings_section_places`). *Mjesto* is the noun `place_sheet_title` uses as well. |
| outside the city served | **Izvan pokrivenog grada** | Under a place the conurbation in use does not cover (`settings_place_outside_city`). *Izvan* and *pokriven* are what `station_beyond_area` already says. This one **may** say *grad*: no placeholder follows it, so the rule that keeps a network's name out of a genitive does not reach it. The application serves several conurbations side by side: a *Dom* named in Lille is out of reach while Zagreb is in use. The place is kept and stays erasable — out of reach, not wrong. |
| Face north and lay the map flat | **Okreni prema sjeveru i poravnaj kartu** | `map_face_north`, on the compass, which shows while the map is turned or tilted, and one press gives back both at once — the north and the flat; `map_bearing_description` counts the turn in *stupnjeva*, over the three categories. |
| tilt / lay flat | **nagnuti / nagnuta**, poravnati | *Nagnuti* is the tilt and *okrenuti* the turn, kept apart so that the four gestures of `map_description` stay four. `map_tilted_description` is „Karta je nagnuta.“, and `map_face_north` says *poravnaj* for laying it flat again. |
| this place | **Ovo mjesto** | `place_sheet_title`, the sheet a point found on the map opens — the same noun as *Moja mjesta*. `place_clear` is *Izbriši ovu točku*. |
| Go there / Leave from here | **Idi onamo / Kreni odavde** | The place sheet's two actions take the station sheet's own words: `station_as_origin` is already *Kreni odavde*, word for word, and `station_as_destination` *Idi ovamo*, of which *Idi onamo* is the same verb with *ovamo* giving way to the far deictic Croatian has and English has not. The place sheet is the station sheet's sister and the two must read as one, so neither pair is reworded without the other. |

...
| Home (the place) | Casa | Where the reader lives, named by them (`settings_place_home`, `journey_source_home`, the same word in both). **Never *Home***, which is what Italian leaves in English for an application's opening screen. |
| Work (the place) | Lavoro | Where they work (`settings_place_work`, `journey_source_work`). The place, not the occupation: it stands beside *Casa* under *I miei luoghi*, which settles the reading; *Luogo di lavoro* is three times the width on a row that already carries an address. |
| My places | I miei luoghi | The settings section that holds the two (`settings_section_places`). *Luogo* is the noun `place_sheet_title` uses as well. |
| outside the city served | Fuori dalla città coperta | Under a place the conurbation in use does not cover (`settings_place_outside_city`). *Fuori da* and *coperto* are what `station_beyond_area` already says. The application serves several conurbations side by side: a *Casa* named in Lille is out of reach while Milano is in use. The place is kept and stays erasable — out of reach, not wrong. |
| Face north and lay the map flat | Orienta a nord e appiattisci la mappa | `map_face_north`, on the compass, which shows while the map is turned or tilted, and one press gives back both at once — the north and the flat; `map_bearing_description` counts the turn in *gradi*. |

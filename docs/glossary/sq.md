...
| metered / unmetered | **me matje / pa matje** | `settings:data_usage_metered_yes` = *Me matje*, `settings:wifi_unmetered_label` = *Pa matje*. `download_unmetered_only` now names Wi-Fi instead — *Shkarko vetëm në Wi-Fi* — exactly as the English does, that being the word every reader already has; the pair is kept for what the code reads and for the line under the switch, which still speaks of billing. |
| may be billed | **mund të faturohet** | The phone knows only that the connection declares itself metered, never what the plan behind it charges, so the three sentences read *mund të faturohet me megabajt* rather than the flat *faturohet*. *Mund të* is Albanian's ordinary modal for a possibility. |
...
| Home (the place) | **Shtëpia** | Where the reader lives, named by them (`settings_place_home`, `journey_source_home`, the same word in both), definite because the row label stands on its own. The dwelling, never an application's opening screen. |
| Work (the place) | **Puna** | Where they work (`settings_place_work`, `journey_source_work`), definite for the same reason. The place, not the occupation: it stands beside *Shtëpia* under *Vendet e mia*, which settles the reading. |
| My places | **Vendet e mia** | The settings section that holds the two (`settings_section_places`). *Vend* is the noun `place_sheet_title` uses as well. |
| outside the city served | **Jashtë qytetit të mbuluar** | Under a place the conurbation in use does not cover (`settings_place_outside_city`). *Jashtë* and *mbuloj* are what `station_beyond_area` already says. This one **may** say *qytet*: no placeholder follows it, so the rule that keeps a network's name out of a definite ending does not reach it. The application serves several conurbations side by side: a *Shtëpia* named in Lille is out of reach while Tirana is in use. The place is kept and stays erasable — out of reach, not wrong. |
| Face north and lay the map flat | **Drejtoje nga veriu dhe rrafshoje hartën** | `map_face_north`, on the compass, which shows while the map is turned or tilted, and one press gives back both at once — the north and the flat, with the clitic *-e* the file's buttons take. `map_bearing_description` counts the turn in *gradë*, which does not change for number, so both plural forms are the same sentence — that is the language, not a slip. |

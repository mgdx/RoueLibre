...
| Face north | **Drejtoje nga veriu** | `map_face_north`, on the compass, which shows only while the map is turned, with the clitic *-e* the file's buttons take. `map_bearing_description` counts the turn in *gradë*, which does not change for number, so both plural forms are the same sentence — that is the language, not a slip. |

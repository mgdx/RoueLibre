...
| metered / unmetered | **maksas / bezmaksas** | Android's own (`settings:wifi_metered_label`, `settings:wifi_unmetered_label`). The switch itself names Wi-Fi — *Lejupielādēt tikai Wi-Fi tīklā*, as the English now does — and *maksas savienojums* is kept for the sentences that explain the billing, `download_unmetered_only_description` spelling out that a shared phone connection counts as a mobile network. **The billing is possible, never asserted**: *var būt maksas savienojums*, since the phone reads only that the connection declares itself metered, never what the plan charges. |
...
| Face north and lay the map flat | **Pagriezt uz ziemeļiem un izlīdzināt karti** | `map_face_north`, on the compass, which shows while the map is turned or tilted, and one press gives back both at once — the north and the flat; the infinitive, as the file's controls are. `map_bearing_description` counts the turn in degrees and carries the `zero` category the section above is about — *grādu*, the genitive plural, for 10 to 19 and for every round ten. |

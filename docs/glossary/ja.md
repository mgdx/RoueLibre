...
| Face north and lay the map flat | 北を上にして地図を平らにする | `map_face_north`, on the compass, which shows while the map is turned or tilted, and one press gives back both at once — the north and the flat. Not 北向き and not 北に回転: what the button does is put the north at the top, which is how a Japanese map application says it. `map_bearing_description` counts the turn in 度. |
| tilt / lay flat | 傾ける / 傾いています, 平らにする | 傾ける is the tilt and 回転 the turn, kept apart so that the four gestures of `map_description` stay four. `map_tilted_description` is 「地図が傾いています。」, and `map_face_north` says 平らにする for the other half of what the press does. The tilt is never given a figure — 度 belongs to the bearing alone. |

...
| metered / unmetered | **s naplatom / bez naplate** | The switch itself names WiFi now, as the English does — *Preuzimaj samo na WiFi-ju* — and the pair is left to the sentences explaining it, which use the system's own: `settings:wifi_metered_label` = *S naplatom*, `settings:wifitrackerlib_wifi_unmetered_label` = *Bez naplate*. Those sentences say the connection **may** be billed, *može se naplaćivati*: the modal *moći* carries the doubt, because the device reads only that the connection declares itself metered, never what the plan behind it charges. |
| Out of service | **Ne radi** | `settings:radioInfo_service_out`. |
| Reserved | **Rezerviran** | On a bike line of the station sheet, where the feed says the bike is booked. Agrees with *bicikl*, as *električan* does on the line above it. |
...
| this place | **Ovo mjesto** | `place_sheet_title`, the sheet a point found on the map opens — the same noun as *Moja mjesta*. `place_clear` is *Obriši ovu tačku*, the verb the row above keeps for what destroys nothing. |
| Go there / Leave from here | **Idi onamo / Kreni odavde** | The place sheet's two actions take the station sheet's own words: `station_as_origin` is already *Kreni odavde*, word for word, and `station_as_destination` *Idi ovamo*, of which *Idi onamo* is the same verb with *ovamo* giving way to the far deictic Bosnian has and English has not. The place sheet is the station sheet's sister and the two must read as one, so neither pair is reworded without the other. |

...
| Home (the place) | Domicile | Where the reader lives, named by them (`settings_place_home`, `journey_source_home`, the same word in both). **Never « Accueil »**, which is an application's first screen and not a place one walks home to. |
| Work (the place) | Travail | Where they work (`settings_place_work`, `journey_source_work`). The place, never the occupation: the row stands beside *Domicile* under *Mes lieux*, which is what settles the reading. « Lieu de travail » says it outright and is three times the width on a row that already carries an address. |
| My places | Mes lieux | The settings section that holds the two (`settings_section_places`). *Lieu* rather than *endroit*, and it is the noun `place_sheet_title` uses, so the section and the sheet name the same kind of thing. |
| outside the city served | Hors de la ville servie | Written under a place the conurbation in use does not cover (`settings_place_outside_city`). *Servie* rather than *couverte*, which `station_beyond_area` keeps for the area the installed data covers: what is at issue here is the city being looked at, not the data. The application serves several conurbations side by side, so a *Domicile* named in Lille is out of reach while Lyon is in use — the place is kept and stays erasable, and the wording says out of reach, not wrong. |
| Face north and lay the map flat | Orienter au nord et remettre la carte à plat | `map_face_north`, the compass button, which shows while the map is turned or tilted, and one press gives back both at once — the north and the flat; `map_bearing_description` says how far, in *degrés*. *Orienter* rather than *Remettre*: nothing is put back to a former state, the map is pointed at the north. |
| tilt / lay flat | incliner / inclinée, à plat | *Incliner* est l’inclinaison, *tourner* la rotation : deux mots distincts, sans quoi les quatre gestes de `map_description` n’en font plus que trois. `map_tilted_description` dit « Carte inclinée. » et `map_face_north` « remettre la carte à plat » — *à plat* plutôt que *horizontale*, qui décrirait un plan et non ce que la vue redevient. L’inclinaison n’est jamais chiffrée : un degré d’inclinaison ne se compare à rien, là où un cap se compare au nord. |
| this place | Ce lieu | `place_sheet_title`, the sheet a point found on the map opens — the same noun as *Mes lieux*. `place_clear` is « Effacer ce point », which keeps *Effacer* for emptying against *Supprimer*, which destroys. |
| Go there / Leave from here | Y aller / Partir d’ici | The place sheet's two actions take the station sheet's own words: `station_as_origin` is already « Partir d’ici », word for word, and `station_as_destination` « Aller ici », of which « Y aller » is the same verb with the deictic moved. The place sheet is the station sheet's sister and the two must read as one, so neither pair is reworded without the other. |

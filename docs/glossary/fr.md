...
| Face north | Orienter au nord | `map_face_north`, the compass button, which shows only while the map is turned; `map_bearing_description` says how far, in *degrés*. *Orienter* rather than *Remettre*: nothing is put back to a former state, the map is pointed at the north. |

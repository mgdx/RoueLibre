...
| Face north and lay the map flat | **Otočiť na sever a vyrovnať mapu** | `map_face_north`, on the compass, which shows while the map is turned or tilted, and one press gives back both at once — the north and the flat; the infinitive, as the file's buttons are. `map_bearing_description` counts the turn in degrees, over all four categories. |
| tilt / lay flat | **nakloniť / naklonená**, vyrovnať | *Nakloniť* is the tilt and *otočiť* the turn, kept apart so that the four gestures of `map_description` stay four. `map_tilted_description` is „Mapa je naklonená.“, and `map_face_north` says *vyrovnať* for laying it flat again. |

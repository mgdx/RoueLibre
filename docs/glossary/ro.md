...
| metered / unmetered | contorizată / necontorizată | Android's own wording for a metered network (`settings:wifi_metered_label`, `settings:wifi_unmetered_label`). The switch itself names Wi-Fi — *Descarcă doar pe Wi-Fi* — exactly as the English does, Wi-Fi being the word every reader already has; the pair stays the name of what the code actually reads, and the line under the switch explains it as billing by the megabyte. |
| may be billed | poate fi taxată | The phone knows only that the connection declares itself metered, never what the plan behind it charges, so the three sentences read *poate fi taxată la megaoctet* and not the flat *este taxată*. The plain modal *a putea* carries the hedge; the conditional *ar putea* would push the risk off into the unlikely, which is not what is meant. |
...
| Home (the place) | Acasă | Where the reader lives, named by them (`settings_place_home`, `journey_source_home`, the same word in both). Romanian labels an application's first screen *Acasă* too; here it never means that, and the row stands under *Locurile mele* beside *Serviciu*, which is what settles the reading. |
| Work (the place) | Serviciu | Where they work (`settings_place_work`, `journey_source_work`). *Serviciu* is the place one goes to — *la serviciu* — where *Muncă* would be the work itself, which is not a point on a map. |
| My places | Locurile mele | The settings section that holds the two (`settings_section_places`). *Loc* is the noun `place_sheet_title` uses as well. |
| outside the city served | În afara orașului acoperit | Under a place the conurbation in use does not cover (`settings_place_outside_city`), genitive after *în afara*. *În afara* and *acoperit* are what `station_beyond_area` already says. The application serves several conurbations side by side: an *Acasă* named in Lille is out of reach while Cluj is in use. The place is kept and stays erasable — out of reach, not wrong. |
| Face north and lay the map flat | Orientează spre nord și aplatizează harta | `map_face_north`, on the compass, which shows while the map is turned or tilted, and one press gives back both at once — the north and the flat, in the *tu* imperative the file's buttons take. `map_bearing_description` counts the turn in degrees, and its third form writes *de grade*, which Romanian requires past nineteen. |

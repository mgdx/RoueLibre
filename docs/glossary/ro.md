...
| Face north | Orientează spre nord | `map_face_north`, on the compass, which shows only while the map is turned, in the *tu* imperative the file's buttons take. `map_bearing_description` counts the turn in degrees, and its third form writes *de grade*, which Romanian requires past nineteen. |

...
| Face north | **Obrni proti severu** | `map_face_north`, on the compass, which shows only while the map is turned; the bare imperative, as the file's buttons are. `map_bearing_description` counts the turn in degrees over all four categories, the dual *stopinji* included — which is the section above, and it is why two degrees may not be written as five. |

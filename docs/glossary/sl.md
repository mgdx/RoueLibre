...
| unmetered / metered | **z neomejenim / omejenim prenosom podatkov** | `settings:wifi_unmetered_label`, `settings:wifitrackerlib_wifi_metered_label` — the plain `settings:wifi_metered_label` gives the shorter *Omejen prenos podatkov*, which is not the form used here. Long, and it is Android's own wording for exactly this distinction; the switch itself, though, names Wi-Fi — *Prenašaj samo prek omrežja Wi-Fi* — as the English does, and the pair is kept for the sentences under it, which still speak of billing. |
| may be billed | **se lahko zaračunava** | The phone knows only that the connection declares itself metered, never what the plan behind it charges, so the sentences read *se lahko zaračunava po megabajtih* and not the flat *se zaračunava*. *Lahko* is the ordinary Slovene hedge for a possibility and does not soften the warning. |
...
| Work (the place) | **Služba** | Where they work (`settings_place_work`, `journey_source_work`). *Služba* is the workplace one goes to; *Delo* would be the work itself, and it is not a point on a map. |
| My places | **Moji kraji** | The settings section that holds the two (`settings_section_places`). *Kraj* is the noun `place_sheet_title` uses as well. |
| outside the city served | **Zunaj pokritega mesta** | Under a place the conurbation in use does not cover (`settings_place_outside_city`). *Zunaj* and *pokrit* are what `station_beyond_area` already says. The application serves several conurbations side by side: a *Dom* named in Lille is out of reach while Ljubljana is in use. The place is kept and stays erasable — out of reach, not wrong. |
| Face north and lay the map flat | **Obrni proti severu in poravnaj zemljevid** | `map_face_north`, on the compass, which shows while the map is turned or tilted, and one press gives back both at once — the north and the flat; the bare imperative, as the file's buttons are. `map_bearing_description` counts the turn in degrees over all four categories, the dual *stopinji* included — which is the section above, and it is why two degrees may not be written as five. |
| tilt / lay flat | **nagniti / nagnjen**, poravnati | *Nagniti* is the tilt and *zavrteti* the turn, kept apart so that the four gestures of `map_description` stay four. `map_tilted_description` is „Zemljevid je nagnjen.“, and `map_face_north` says *poravnaj* for laying it flat again. `map_bearing_tilted_description` carries the dual *stopinji* like its sister string. |
| this place | **Ta kraj** | `place_sheet_title`, the sheet a point found on the map opens — the same noun as *Moji kraji*. `place_clear` is *Počisti to točko*, the third verb the row above keeps for emptying without destroying. |
| Go there / Leave from here | **Pojdi tja / Začni tukaj** | The place sheet's two actions take the station sheet's own words: `station_as_origin` is already *Začni tukaj*, word for word, and `station_as_destination` *Pojdi sem*, of which *Pojdi tja* is the same verb with the deictic moved. The place sheet is the station sheet's sister and the two must read as one, so neither pair is reworded without the other. |

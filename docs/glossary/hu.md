...
| Face north | **Északra tájolás** | `map_face_north`, on the compass, which shows only while the map is turned; nominal, as the file's controls are. `map_bearing_description` counts the turn in *fokkal*, instrumental, and its two plural forms are the same sentence because a Hungarian numeral leaves the noun singular. |

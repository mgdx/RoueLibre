...
| Home (the place) | **Otthon** | Where the reader lives, named by them (`settings_place_home`, `journey_source_home`, the same word in both). **Never *Kezdőlap* or *Főoldal***: an application's opening screen is not a place one cycles home to. |
| Work (the place) | **Munkahely** | Where they work (`settings_place_work`, `journey_source_work`). *Munkahely* names the workplace outright, which is what the row means; *Munka* would be the work itself, and it is not a point on a map. |
| My places | **Saját helyek** | The settings section that holds the two (`settings_section_places`). *Saját* rather than *Az én*, which is what Hungarian writes for a section of one's own things; *hely* is the noun `place_sheet_title` uses as well. |
| outside the city served | **A lefedett városon kívül** | Under a place the conurbation in use does not cover (`settings_place_outside_city`). *Lefedett* and the postposition *kívül* are what `station_beyond_area` already says. The application serves several conurbations side by side: an *Otthon* named in Lille is out of reach while Budapest is in use. The place is kept and stays erasable — out of reach, not wrong. |
| Face north and lay the map flat | **Északra tájolás és a térkép vízszintbe állítása** | `map_face_north`, on the compass, which shows while the map is turned or tilted, and one press gives back both at once — the north and the flat; nominal, as the file's controls are. `map_bearing_description` counts the turn in *fokkal*, instrumental, and its two plural forms are the same sentence because a Hungarian numeral leaves the noun singular. |

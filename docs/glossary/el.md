...
| Face north and lay the map flat | Προσανατολισμός στον βορρά και επίπεδη προβολή | `map_face_north`, on the compass, which shows while the map is turned or tilted, and one press gives back both at once — the north and the flat; nominal, as the file's controls are. `map_bearing_description` counts the turn in *μοίρες*. |

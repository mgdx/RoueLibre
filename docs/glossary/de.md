...
| Face north and lay the map flat | Nach Norden ausrichten und flach legen | `map_face_north`, on the compass, which appears while the map is turned or tilted, and one press gives back both at once — the north and the flat; `map_bearing_description` says how far, in *Grad*. *Grad* does not inflect after a numeral, so both plural forms of that string are the same sentence — that is the language, not a slip. |

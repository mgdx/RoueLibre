...
| Face north and lay the map flat | Op het noorden richten en de kaart vlak leggen | `map_face_north`, on the compass, which shows while the map is turned or tilted, and one press gives back both at once — the north and the flat; `map_bearing_description` counts the turn in *graden*. |

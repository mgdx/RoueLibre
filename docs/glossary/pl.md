...
| Face north | Skieruj na północ | `map_face_north`, on the compass, which shows only while the map is turned; the bare imperative, as the file's buttons are. `map_bearing_description` counts the turn in degrees, over all four categories. |

...
| unmetered / metered | bez naliczania opłat / rozliczane za megabajty | Android's Polish for a metered connection is a whole sentence — *Użycie danych jest mierzone* (`settings:wifi_metered_label`) — which does not fit a switch label. The switch now names Wi-Fi instead, *Pobieraj tylko przez Wi-Fi*, as the English does, so *bez naliczania opłat* has left the screen; *rozliczane za megabajty* stays in the messages, which still say what is billed. **The billing is possible, never asserted**: *może być rozliczane za megabajty*, since the phone reads only that the connection declares itself metered, never what the plan charges. |
...
| Home (the place) | Dom | Where the reader lives, named by them (`settings_place_home`, `journey_source_home`, the same word in both). **Never *Strona główna* or *ekran główny***: an application's first screen is not a place one rides home to. |
| Work (the place) | Praca | Where they work (`settings_place_work`, `journey_source_work`). The place, not the occupation: it stands beside *Dom* under *Moje miejsca*, which settles the reading; *Miejsce pracy* is twice the width on a row that already carries an address. |
| My places | Moje miejsca | The settings section that holds the two (`settings_section_places`). *Miejsce* is the noun `place_sheet_title` uses as well. |
| outside the city served | Poza obsługiwanym miastem | Under a place the conurbation in use does not cover (`settings_place_outside_city`). *Obsługiwane*, served, rather than *objęte*, which `station_beyond_area` keeps for the area the installed data covers: the English tells the city served from the area covered, and so does this. The application serves several conurbations side by side: a *Dom* named in Lille is out of reach while Warszawa is in use. The place is kept and stays erasable — out of reach, not wrong. |
| Face north and lay the map flat | Skieruj na północ i wypoziomuj mapę | `map_face_north`, on the compass, which shows while the map is turned or tilted, and one press gives back both at once — the north and the flat; the bare imperative, as the file's buttons are. `map_bearing_description` counts the turn in degrees, over all four categories. |
| tilt / lay flat | pochylić / pochylona, wypoziomować | *Pochylić* is the tilt and *obrócić* the turn, kept apart so that the four gestures of `map_description` stay four. `map_tilted_description` is „Mapa jest pochylona.”, and `map_face_north` says *wypoziomuj* for laying it flat again. |
| this place | To miejsce | `place_sheet_title`, the sheet a point found on the map opens — the same noun as *Moje miejsca*. `place_clear` is *Usuń ten punkt*, the bare *Usuń* the row above describes. |
| Go there / Leave from here | Jedź tam / Wyrusz stąd | The place sheet's two actions take the station sheet's own words: `station_as_origin` is already *Wyrusz stąd*, word for word, and `station_as_destination` *Jedź tutaj*, of which *Jedź tam* is the same verb with the deictic moved. The place sheet is the station sheet's sister and the two must read as one, so neither pair is reworded without the other. |

...
| Delete / Remove | **Sil** / **Kaldır** | Turkish keeps the two apart and so does this file. *Sil* destroys (`android:delete`, `settings:delete`): `city_delete`, `dataset_delete`, `dataset_delete_description`. *Kaldır* takes out of a list (`settings:remove`, `android:kg_reordering_delete_drop_target_text`): `station_favourite_remove`, *Favorilerden kaldır*. |
| Clear (a search) | **Aramayı temizle** | Android's verb for emptying a field (`settings:proxy_clear_text`, `settings:lockpattern_retry_button_text`). One wording serves the icon inside the field, the button in the empty state and the city screen alike. `place_clear` belongs here too, and **not** under *Sil*: *Bu noktayı temizle* empties a selection rather than destroying anything, which is what the English "clear this point" says. |
...
| metered / unmetered | **sayaçlı / sayaçsız** | Android labels a connection *Sayaçlı* (`settings:wifi_metered_label`, `settings:data_usage_metered_yes`); *sayaçsız* is its regular negative. The switch itself no longer takes the pair: it names the connection — *Yalnızca kablosuz bağlantıda indir* — where the English now names Wi-Fi, the word every reader already has, and the row below gives what that word is in this file. The pair stays the name of what the code reads, and the sentences under the switch say what is billed, *megabayt başına ücretlendirilebilir*. |
| may be billed | **ücretlendirilebilir** | The phone knows only that the connection declares itself metered, never what the plan behind it actually charges, so the three sentences take the ability suffix *-ebil* — *megabayt başına ücretlendirilebilir* — rather than the flat *ücretlendiriliyor*. Turkish carries the whole hedge inside the verb, with no adverb added. |
...
| Home (the place) | **Ev** | Where the reader lives, named by them (`settings_place_home`, `journey_source_home`, the same word in both). **Never *Ana ekran* or *Ana sayfa***: an application's opening screen is not a place one rides home to. |
| Work (the place) | **İş** | Where they work (`settings_place_work`, `journey_source_work`), with the dotted *İ* the first section of this file is about. The place, not the occupation: it stands beside *Ev* under *Yerlerim*, which settles the reading; *İş yeri* names the workplace outright and is three times the width on a row that already carries an address. |
| My places | **Yerlerim** | The settings section that holds the two (`settings_section_places`). The possessive is the suffix *-im*, not a word of its own; *yer* is the noun `place_sheet_title` uses as well. |
| outside the city served | **Kapsanan şehrin dışında** | Under a place the conurbation in use does not cover (`settings_place_outside_city`). *Kapsamak* and the postposition *dışında* are what `station_beyond_area` already says. The application serves several conurbations side by side: an *Ev* named in Lille is out of reach while İstanbul is in use. The place is kept and stays erasable — out of reach, not wrong. |
| Face north and lay the map flat | **Kuzeye çevir ve haritayı düzleştir** | `map_face_north`, on the compass, which shows while the map is turned or tilted, and one press gives back both at once — the north and the flat, in the bare *sen* imperative the file's buttons take. `map_bearing_description` counts the turn in *derece*, which a numeral leaves singular, so both plural forms are the same sentence — that is the language, not a slip. |

...
| Face north | **Kuzeye çevir** | `map_face_north`, on the compass, which shows only while the map is turned, in the bare *sen* imperative the file's buttons take. `map_bearing_description` counts the turn in *derece*, which a numeral leaves singular, so both plural forms are the same sentence — that is the language, not a slip. |

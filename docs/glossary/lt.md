...
| unmetered / metered | neapmokestinamas / apmokestinamas | **A departure from Android**, which says *Nematuojamas* / *Matuojamas* (`settings:wifitrackerlib_wifi_unmetered_label`, `settings:wifi_metered_label`) — literally un/measured. The switch itself names Wi-Fi — *Atsisiųsti tik per „Wi-Fi“*, as the English now does — and the pair is kept for the sentences that explain **what is billed**, `download_unmetered_only_description` first among them; *nematuojamas* would name the measuring instead and lose the point. Android's own second rendering, `settings:wifi_unmetered_label` → *Neišmatuotas*, shows the term is not settled there either. **The billing is possible, never asserted**: *gali būti apmokestinamas*, since the phone reads only that the connection declares itself metered. |
...
| Home (the place) | Namai | Where the reader lives, named by them (`settings_place_home`, `journey_source_home`, the same word in both). Plural in form and singular in sense, as Lithuanian has it. Never an application's opening screen. |
| Work (the place) | Darbas | Where they work (`settings_place_work`, `journey_source_work`). The place, not the occupation: it stands beside *Namai* under *Mano vietos*, which settles the reading; *Darbovietė* names the workplace outright and is twice the width on a row that already carries an address. |
| My places | Mano vietos | The settings section that holds the two (`settings_section_places`). *Vieta* is the noun `place_sheet_title` uses as well. |
| outside the city served | Už aptarnaujamo miesto ribų | Under a place the conurbation in use does not cover (`settings_place_outside_city`), in the *už … ribų* frame `station_beyond_area` already uses. *Aptarnaujamas*, served, rather than *apimamas*, which the station line keeps for the area the installed data covers. The application serves several conurbations side by side: *Namai* named in Lille are out of reach while Vilnius is in use. The place is kept and stays erasable — out of reach, not wrong. |
| Face north and lay the map flat | Nukreipti į šiaurę ir ištiesinti žemėlapį | `map_face_north`, on the compass, which shows while the map is turned or tilted, and one press gives back both at once — the north and the flat; the infinitive, as the file's controls are. `map_bearing_description` counts the turn in degrees, over the four categories Lithuanian has. |

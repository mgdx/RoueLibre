...
| Face north | Nukreipti į šiaurę | `map_face_north`, on the compass, which shows only while the map is turned; the infinitive, as the file's controls are. `map_bearing_description` counts the turn in degrees, over the four categories Lithuanian has. |

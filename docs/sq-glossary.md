...
| map | **harta** | Also the name of the tiles dataset, `dataset_tiles`; see below. |
...
| `station_address_nearby` | *Afër %1$s* | Same: *afër* governs the ablative but accepts a bare name, and the argument here is a street **or** a square. |
...
So the three names are **Harta**, **Grafi i itinerarëve** and **Indeksi i
adresave**, all singular, and the three lines above read correctly for each of
them. *Harta* alone is enough on that screen, where three installable things
are listed with their sizes and their purposes beneath them, and the English
repeats "map" between the name and the purpose in the same way.
...
**One deliberate departure from the English.** `full_description` lists the
cities served, and the English list runs "Prague, Barcelona, Copenhagen, Tokyo
and Buenos Aires". The Albanian list opens that run with **Prishtinë**, because
Prishtina bike is the one network this language actually serves and its data is
published (`config/catalogue.json`, `prishtina-bike`). Nothing else was added
or dropped. `changelogs/3.txt` needed no such decision: the English already
names Pristina there.
